# Video Game Audio Designer repository instructions

This repository contains one standalone C-Sweet protocol-v2 agent. Its purpose is:

> Owns audio direction, assets or briefs, implementation metadata, loudness, looping, and accessibility.

## Invariants

- Keep `com.csweet.video-game-audio-designer` and version `2.2.0` synchronized between agent code,
...
```powershell
dotnet test
dotnet run --project src/CSweet.Agent.AudioDesigner.VideoGame -- --self-test
```
